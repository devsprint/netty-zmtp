...
public class ZMQIntegrationTest {
	...
	public void setup() {
		...
		serverBootstrap.setPipelineFactory(new ChannelPipelineFactory() {
			Executor executor = new OrderedMemoryAwareThreadPoolExecutor(Runtime.getRuntime().availableProcessors(),
					1024 * 1024, 128 * 1024 * 1024);

			public ChannelPipeline getPipeline() throws Exception {
				final ZMTPSession session = new ZMTPSession(ZMTPConnectionType.Addressed, identity.getBytes());

				return Channels.pipeline(new ExecutionHandler(executor), new ZMTPFramingDecoder(session),
						new ZMTPFramingEncoder(session), new SimpleChannelUpstreamHandler() {
	...
	public void testZmqRouter() throws Exception {
		...
		final ZMTPMessage request = new ZMTPMessage(asList(ZMTPFrame.create("envelope")), asList(
				ZMTPFrame.create("hello"), ZMTPFrame.create("world")));
