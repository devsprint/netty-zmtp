...
/**
 * A channel handler that attempts to batch together and consolidate smaller
 * writes to avoid many small individual writes on the channel and the syscall
...
public class AutoFlushingWriteBatcher extends BufferedWriteHandler {
	...
	private static final ScheduledThreadPoolExecutor flusher = new ScheduledThreadPoolExecutor(1, new ThreadFactory() {
		@NotNull
		@Override
	...
	/**
	 * Scheduled to be called regularly to enforce the max delay of outgoing
	 * messages in the buffer.
	 */
	private final Runnable flushTask = new Runnable() {
		@Override
		public void run() {
			// Flush if the buffer has not been flushed during the last max
			// delay time interval
	...
	/**
	 * Create a write batcher with custom flushing interval, i.e. the maximum
	 * amount of time between a message being written to the buffer and the
	 * buffer being flushed.
	 * 
	 * @param interval
	 *            The flush interval.
	 * @param intervalUnit
	 *            The time unit of the flush interval.
	 */
	...
	/**
	 * Create a write batcher, controlling whether it consolidates buffers when
	 * flushing. I.e., if the channel buffers are joined and written to a single
	 * channel buffer when flushing. NIO scatter writes are slow enough that
	 * consolidating buffers seem to always be the most performant option.
	 * 
	 * @param consolidateOnFlush
	 *            true if buffers should be consolidated on flush, false
	 *            otherwise.
	 */
	 ...
	 * Create a write batcher with custom flushing interval, i.e. the maximum
	 * amount of time between a message being written to the buffer and the
	 * buffer being flushed as well as if the buffers should be consolidated
	 * when flushing. I.e., if the channel buffers are joined and written to a
	 * single channel buffer when flushing. NIO scatter writes are slow enough
	 * that consolidating buffers seem to always be the most performant option.
	 * 
	 * @param interval
	 *            The flush interval.
	 * @param intervalUnit
	 ...
	 *            otherwise.
	 */
	public AutoFlushingWriteBatcher(final long interval, final TimeUnit intervalUnit, final boolean consolidateOnFlush) {
	...
	public void channelOpen(final ChannelHandlerContext ctx, final ChannelStateEvent e) throws Exception {
		...
		// Schedule a task to flush and enforce the maximum latency that a
		// message is buffered
	...
	public void writeRequested(final ChannelHandlerContext ctx, final MessageEvent e) throws Exception {
		...
		// Flush if writes are sparse or if the buffer has reached its threshold
		// size
		if (nanosSinceLastWrite > maxDelayNanos || newBufferSize > maxBufferSize) {
			flush();
		}
	}
